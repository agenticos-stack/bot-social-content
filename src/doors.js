...
/**
 * `env.social.uploadMedia(input)` — generated bytes become a hosted,
 * publisher-addressable asset. The door returns `{ assetId, url }` for a
 * public capability URL the provider's fetcher can reach. Same absent-door
 * contract as `socialCreateDraft`: throw when the method is missing.
 */
export async function socialUploadMedia(env, input) {
  const social = env && env.social;
  if (!social || typeof social.uploadMedia !== "function") {
    throw new Error("The Social Hub door does not accept media uploads in this deployment.");
  }
  return social.uploadMedia(input);
}
